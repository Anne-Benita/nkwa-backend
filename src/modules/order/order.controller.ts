...
export const orderController = {
   ...
   */
  getAvailable: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
  ...
  accept: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!orderId) {
        return next(new AppError(400, 'VALIDATION_ERROR', 'Order ID parameter is required'));
      }

      const order = await orderService.acceptOrder(req.driver.id, orderId);
  ...
  getCurrent: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const orders = await orderService.getCurrentOrders(req.driver.id);
      res.status(200).json(orders);
    } catch (err) {
      next(err);
    }
  ...
  getDetails: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const order = await orderService.getOrderDetails(req.driver.id, orderId);
      res.status(200).json(order);
    } catch (err) {
      next(err);
    }
      ...
      if (!status || !['PICKED_UP', 'EN_ROUTE', 'ARRIVED', 'DELIVERED', 'CANCELLED'].includes(status.toUpperCase())) {
        return next(
          new AppError(
            400,
            'VALIDATION_ERROR',
            'status must be one of PICKED_UP, EN_ROUTE, ARRIVED, DELIVERED, or CANCELLED',
            'status'
          )
        );
      }

      const updatedOrder = await orderService.updateOrderStatus(req.driver.id, orderId, status);
   ...
   */
  getTracking: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const orderId = req.params.orderId as string;
      if (!orderId) {
        return next(new AppError(400, 'VALIDATION_ERROR', 'Order ID parameter is required'));
  ...
  getMessages: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const messages = await orderService.getOrderMessages(orderId, page, limit);
  ...
  sendMessage: async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!body || typeof body !== 'string') {
        return next(new AppError(400, 'VALIDATION_ERROR', 'Message body is required and must be a string', 'body'));
      }

      const message = await orderService.sendOrderMessage(orderId, 'DRIVER', req.driver.id, body);
